...
if (!$solicitud) {
    echo "<script>alert('Solicitud no encontrada'); window.location='listarsolicitudes.php';</script>";
    exit;
}

// Obtener incidencias para el modal de motivo (mismo que formsolicitudinsumos.php)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $emisor_dep = $_POST['emisor_dep'];
    $receptor_dep = $_POST['receptor_dep'];
    $descripcion = $_POST['descripcion'];
    $materiales = $_POST['materiales'] ?? [];

    // Actualizar tipo de solicitud
    $id_incidencia = ($_POST['tipo_solicitud'] == 'incidencia' && !empty($_POST['id_incidencia'])) ? $_POST['id_incidencia'] : NULL;
    $razon_manual = ($_POST['tipo_solicitud'] == 'manual' && !empty($_POST['razon_manual'])) ? $_POST['razon_manual'] : NULL;
    ...
    if (empty($materiales)) {
        ...
        if (!method_exists($solicitudObj, 'actualizarDetalles')) {
            $mensaje = "Error: Método actualizarDetalles no encontrado en la clase Solicitud";
            $tipo_mensaje = "error";
        } else {
            try {
                // Iniciar transacción
                ...
                // Actualizar datos principales de la solicitud
                $sql = "UPDATE solicitud_materiales 
                        SET fecha = ?, receptor = ?, departamento_emisor = ?, departamento_destino = ?, 
                            descripcion = ?, id_incidencia = ?, razon_manual = ? 
                        WHERE id_solicitud = ?";
                ...
                if (!$resultado) {
                    throw new Exception("Error al actualizar la solicitud principal");
                }

                // Procesar los materiales usando el método de la clase
                $resultado_detalles = $solicitudObj->actualizarDetalles($id_solicitud, $materiales);

                if (!$resultado_detalles) {
                    throw new Exception("Error al actualizar los detalles de la solicitud");
                }
                ...
                echo "<script>
                        ...
                        window.location='listarsolicitudes.php';
                      </script>";
                exit;
                ...
                $tipo_mensaje = "error";
                error_log("Error en edición de solicitud: " . $e->getMessage());
            }
        }
    }
}

// Determinar tipo de solicitud actual para edición
$tipo_solicitud_actual = $solicitud->id_incidencia ? 'incidencia' : 'manual';
?>
<!DOCTYPE html>
<html>
<head>
    ...
    <title>Editar Solicitud de Material</title>
    <style>
        /* RESET Y VARIABLES - IGUAL QUE formsolicitudinsumos.php */
        * {
        ...
        /* MODAL - IGUAL QUE formsolicitudinsumos.php */
        #modal-overlay {
            position: fixed;
            top: 0;
            left: 0;
        ...
        <form method="post" action="" id="solicitudForm">
            <div class="inline-fields">
                <div class="form-group">
                    ...
                    <input type="date" name="fecha" id="fecha" value="<?= htmlspecialchars($solicitud->fecha) ?>" required readonly>
                </div>
                <div class="form-group">
                    <label for="receptor">Receptor: *</label>
                    <input type="text" name="receptor" id="receptor" value="<?= htmlspecialchars($solicitud->receptor) ?>" required placeholder="Nombre del receptor">
                </div>
            </div>
            ...
            <div class="inline-fields">
                <div class="form-group">
                    ...
                    <select name="emisor_dep" id="emisor_dep" required>
                        ...
                        <?php foreach ($departamentos as $dep): ?>
                            <option value="<?= $dep['id_departamento'] ?>" <?= $dep['id_departamento'] == $solicitud->departamento_emisor ? 'selected' : '' ?>>
                ...
                <div class="form-group">
                    ...
                    <select name="receptor_dep" id="receptor_dep" required>
                        <option value="">Seleccione...</option>
                        <?php foreach ($departamentos as $dep): ?>
                            <option value="<?= $dep['id_departamento'] ?>" <?= $dep['id_departamento'] == $solicitud->departamento_destino ? 'selected' : '' ?>>
                                <?= htmlspecialchars($dep['nombre']) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
            ...
            <!-- Motivo de la solicitud - IGUAL QUE formsolicitudinsumos.php -->
            <div class="motivo-solicitud">
                ...
                <div class="radio-group">
                    <label class="radio-option">
                        <input type="radio" id="tipo_incidencia" name="tipo_solicitud" value="incidencia" <?= $tipo_solicitud_actual == 'incidencia' ? 'checked' : '' ?> onchange="mostrarCampoSolicitud()">
                        <span>Asociar a Incidencia</span>
                    </label>
                    <label class="radio-option">
                        <input type="radio" id="tipo_manual" name="tipo_solicitud" value="manual" <?= $tipo_solicitud_actual == 'manual' ? 'checked' : '' ?> onchange="mostrarCampoSolicitud()">
                        <span>Otra Razón</span>
                    </label>
                </div>

                <div class="campo-dinamico <?= $tipo_solicitud_actual == 'incidencia' ? 'active' : '' ?>" id="campo-incidencia">
                    <div class="form-group">
                        ...
                        <select name="id_incidencia" id="id_incidencia" onchange="mostrarInfoIncidencia()">
                            ...
                            <?php foreach ($incidencias as $inc): ?>
                                <option value="<?= $inc['id_incidencia'] ?>" 
                                        ...
                                        <?= $solicitud->id_incidencia == $inc['id_incidencia'] ? 'selected' : '' ?>>
                ...
                <div class="campo-dinamico <?= $tipo_solicitud_actual == 'manual' ? 'active' : '' ?>" id="campo-manual">
                    <div class="form-group">
                        <label for="razon_manual">Justificar Solicitud:</label>
                        <textarea name="razon_manual" id="razon_manual" rows="3" placeholder="Explique el motivo de esta solicitud..."><?= htmlspecialchars($solicitud->razon_manual ?? '') ?></textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <textarea name="descripcion" id="descripcion" rows="3" placeholder="Describa brevemente la solicitud..."><?= htmlspecialchars($solicitud->descripcion) ?></textarea>
            </div>

            <!-- Sección de insumos - IGUAL QUE formsolicitudinsumos.php -->
                ...
                <div class="table-container">
                    <table id="tablaMateriales">
                        ...
                        <tbody>
                            ...
                            <tr id="fila-<?= $i ?>">
                                <td>
                                    <input type="text" name="materiales[<?= $i ?>][codigo]" value="<?= htmlspecialchars($det->id_insumo . ' - ' . $det->nombre) ?>" readonly style="background: #f3f4f6;">
                                    <input type="hidden" name="materiales[<?= $i ?>][id_insumo]" value="<?= $det->id_insumo ?>">
                                    <input type="hidden" name="materiales[<?= $i ?>][id_detalle]" value="<?= $det->id_detalle ?>">
                                </td>
                                <td><input type="text" name="materiales[<?= $i ?>][unidad]" value="<?= htmlspecialchars($det->unidad) ?>" readonly style="background: #f3f4f6;"></td>
                                <td><input type="number" name="materiales[<?= $i ?>][cantidad_pedida]" value="<?= htmlspecialchars($det->cantidad_pedida) ?>" step="1" min="1" required placeholder="Cant"></td>
                                <td>
                                    <?php if ($det->imagen): ?>
                                        <img src="../assets/imagenes/insumos/<?= htmlspecialchars($det->imagen) ?>" class="insumo-img" onerror="this.style.display='none'; this.nextElementSibling.style.display='block';" style="display: block;">
                                        <div style="display: none; color: #6b7280; font-size: 11px;">Sin img</div>
                                    <?php else: ?>
                                        <div style="color: #6b7280; font-size: 11px;">Sin img</div>
                                    <?php endif; ?>
                                    <input type="hidden" name="materiales[<?= $i ?>][cantidad_recibida]" value="<?= htmlspecialchars($det->cantidad_recibida) ?>">
                                </td>
            ...
            <div class="btn-container">
                ...
                <a href="listarsolicitudes.php" class="btn btn-primary" style="background-color: #6c757d !important;">
...
function seleccionarInsumo(id, nombre, unidad, imagen) {
    ...
    nuevaFila.innerHTML = `
        <td>
            ...
            <!-- IMPORTANTE: No incluir id_detalle para nuevos materiales -->
        </td>
        <td><input type="text" name="materiales[${filaActualIndex}][unidad]" value="${unidad}" readonly style="background: #f3f4f6;"></td>
        <td><input type="number" name="materiales[${filaActualIndex}][cantidad_pedida]" step="1" min="1" required placeholder="Cant"></td>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (document.getElementById('tipo_incidencia').checked) {
