...
$insumo_nombre = $_GET['insumo_nombre'] ?? '';
$cantidad_sugerida = $_GET['cantidad_sugerida'] ?? '';
...
$nombre_departamento_usuario = '';
if ($id_trabajador) {
    $sql_departamento = "SELECT d.id_departamento, d.nombre 
                         FROM trabajadores t 
                         JOIN departamentos d ON t.id_departamento = d.id_departamento 
                         WHERE t.id_trabajador = ?";
    $stmt_departamento = $conexion->prepare($sql_departamento);
    $stmt_departamento->execute([$id_trabajador]);
    $departamento_usuario = $stmt_departamento->fetch(PDO::FETCH_ASSOC);
    
    if ($departamento_usuario) {
        $id_departamento_usuario = $departamento_usuario['id_departamento'];
        $nombre_departamento_usuario = $departamento_usuario['nombre'];
    }
}

$descripcion_predefinida = '';
if ($origen === 'alerta_stock' && !empty($insumo_nombre)) {
    $descripcion_predefinida = "Solicitud de reposición de stock para: " . $insumo_nombre;
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $salida = new SalidaAlmacen($conexion);
    $salida->fecha = $_POST['fecha'];
    $salida->emisor = $emisorNombre;
    $salida->departamento_emisor = $_POST['emisor_dep'];
    $salida->receptor = $_POST['receptor'];
    $salida->departamento_destino = $_POST['receptor_dep'];
    ...
    if (empty($materiales)) {
        ...
        $id_solicitud = $salida->crearSalida();
        if ($id_solicitud) {
            $salida->agregarDetalle($id_solicitud, $materiales);
            $mensaje = "Solicitud registrada exitosamente.";
            $tipo_mensaje = "exito";
            ?>
            <script>
                window.open('imprimirsalida.php?id=<?= $id_solicitud ?>', '_blank');
            </script>
            <?php
        } else {
...
<head>
    ...
    <title>Nueva Solicitud de Compra desde Almacén</title>
    <style>
        /* RESET Y VARIABLES */
        * {
...
<div class="form-wrapper" id="formContainer">
    ...
    <div class="title-card">
        <h1>
            <img src="../assets/resources/inventario.png" alt="Almacén">
            NUEVA SOLICITUD DE COMPRA DESDE ALMACÉN
            <?php if ($origen === 'alerta_stock'): ?>
                <span class="info-badge">Desde Alerta de Stock</span>
            <?php endif; ?>
        </h1>
        <p>Complete el formulario para crear una nueva solicitud de compra</p>
    </div>
    ...
    <div class="form-card">
        ...
        <?php if ($origen === 'alerta_stock' && !empty($insumo_nombre)): ?>
            <div class="alert alert-info">
                 <strong>Insumo pre-cargado:</strong> <?= htmlspecialchars($insumo_nombre) ?>
                <?php if (!empty($nombre_departamento_usuario)): ?>
                    | <strong>Departamento:</strong> <?= htmlspecialchars($nombre_departamento_usuario) ?>
                <?php endif; ?>
            </div>
        <?php endif; ?>

        <form method="post" action="" id="salidaForm">
...
<?php
// Buscar el ID del departamento de Compras
$stmt = $conexion->prepare("SELECT id_departamento FROM departamentos WHERE nombre LIKE '%Compras%' LIMIT 1");
$stmt->execute();
$deptoCompras = $stmt->fetch(PDO::FETCH_ASSOC);
$idCompras = $deptoCompras ? $deptoCompras['id_departamento'] : 3; // Fallback a 3 si no encuentra
?>
...
<div class="inline-fields">
        ...
        <select name="emisor_dep" id="emisor_dep" required>
            ...
            <?php foreach ($departamentos as $dep): ?>
                <option value="<?= $dep['id_departamento'] ?>" 
                    <?= ($dep['id_departamento'] == $id_departamento_usuario) ? 'selected' : '' ?>>
    ...
    <div class="form-group">
        ...
        <select name="receptor_dep_display" id="receptor_dep" required disabled>
            <option value="<?= $idCompras ?>" selected>Compras</option>
        </select>
        <input type="hidden" name="receptor_dep" value="<?= $idCompras ?>">
    </div>
</div>
            ...
            <div class="form-group">
                ...
                <textarea name="descripcion" id="descripcion" rows="3" placeholder="Describa brevemente la solicitud..." required><?= htmlspecialchars($descripcion_predefinida) ?></textarea>
            </div>
...
function agregarFila(autoCompletar = false) {
    ...
    <?php if (!empty($insumo_id)): ?>
    if (autoCompletar && index === 0) {
        setTimeout(() => {
            seleccionarInsumo(
                '<?= addslashes($insumo_id) ?>',
                '<?= addslashes($insumo_nombre) ?>',
                '<?= addslashes($unidad_medida) ?>',
                '',
                '<?= $cantidad_sugerida ?>'
            );
        }, 100);
...
function cargarCategorias() {
    fetch('../insumos/buscar_insumos_ajax.php?accion=categorias')
        .then(res => res.json())
        .then(categorias => {
...
function buscarInsumos() {
    ...
    fetch(`../insumos/buscar_insumos_ajax.php?accion=buscar&pagina=${paginaActual}&termino=${encodeURIComponent(termino)}&categoria=${categoria}`)
        .then(res => res.json())
...
function seleccionarInsumo(id, nombre, unidad, imagen, cantidadSugerida = '') {
    const tbody = document.querySelector("#tablaMateriales tbody");
    ...
    nuevaFila.innerHTML = `
        ...
        <td><input type="number" name="materiales[${filaActualIndex}][cantidad_solicitada]" step="0.01" min="0.01" required placeholder="Cant" value="${cantidadSugerida}"></td>
    ...
    setTimeout(() => {
        const input = document.querySelector(`input[name="materiales[${filaActualIndex}][cantidad_solicitada]"]`);
        if (input) {
            input.focus();
            input.select();
        }
...
function seleccionarManual() {
    ...
    nuevaFila.innerHTML = `
        ...
        <td><input type="number" name="materiales[${filaActualIndex}][cantidad_solicitada]" step="0.01" min="0.01" required placeholder="Cant"></td>
...
document.getElementById('salidaForm').addEventListener('submit', function(e) {
    ...
    document.querySelectorAll('input[name$="[cantidad_solicitada]"]').forEach(input => {
    ...
    if (!confirm("¿Está seguro de crear esta solicitud de compra?")) {
        e.preventDefault();
    }
});
...
document.addEventListener('input', function(e) {
    if (e.target.type === 'number' && e.target.name?.includes('cantidad_solicitada')) {
